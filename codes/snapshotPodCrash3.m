...
function [phiVecForModeM]=snapshotPodCrash3(currentAzimuthalMode,currentCrossSection,currentCorrMat,currentUvec )
%function pod()
%figure(1);
[phiVecForModeM]=initData2("pvfmM");
...
for rr=1:ss
    % this needs to be bloc-length loopified. 
    % eg for tt=1:ntimesteps*blocLength
for tt=1:ntimesteps*blocLength % finding the n eigenfunctions Phi^{(n)}(r)...:
    % currentUvec.r(7).dat  
    % disable for crash3 aa=currentUvec.r(rr).dat(tt); % this indeed varies with t.

    % currentUVec shouldnt be zero.
    aa=currentUvec(rr).dat(tt); % this indeed varies with t.
...
%phiVecForModeM(podModeNumber).dat(rr) = ad/(eigVal(tt,tt)*(ntimesteps*blocLength)); % smits.eq.2.5
phiVecForModeM(podModeNumber).dat(rr) = ad/(eigVal(podModeNumber,podModeNumber)*(ntimesteps*blocLength)); % smits.eq.2.5
